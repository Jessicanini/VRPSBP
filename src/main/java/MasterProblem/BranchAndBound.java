...
public class BranchAndBound {
    ...
    double lowerbound;
    double upperbound;
    FileWriter fw = new FileWriter("./dataset/BpLog.txt");
    ...
    public BranchAndBound(BufferedWriter bw1, BufferedWriter bw2, BufferedWriter bw3,
                          BufferedWriter bw4) throws IOException {
        lowerbound = -1E10;
        upperbound = 1E10;
        this.bw1 = bw1;
        this.bw2 = bw2;
    ...
    public boolean BBnode(paramsVRP userParam, ArrayList<route> routes,
                          treeBB branching, ArrayList<route> bestRoutes, int depth, boolean isOnePass,
                          boolean isMaxcnt,boolean isPrediction)
            throws IOException {
        ...
        try {
            ...
            /*pw.print("Java Memory=> Total:" + (runtime.totalMemory() / mb)
                    + " Max:" + (runtime.maxMemory() / mb) + " Used:"
                    + ((runtime.totalMemory() - runtime.freeMemory()) / mb) + " Free: "
                    + runtime.freeMemory() / mb);*/
            // Compute a solution for this node using Column generation
            columnGeneration2 CG = new columnGeneration2();
            CGobj = CG.computeColGen(userParam, routes, bw1, bw2, bw3, bw4, isOnePass, isMaxcnt,isPrediction);
            System.err.println("CGobj = " + CGobj);
            //FVCGobj = 0;
            // feasible ? Does a solution exist?
            double sumProfit = -1 * Arrays.stream(userParam.profit).sum();
            if ((CGobj > 2 * userParam.maxlength) || (CGobj < sumProfit)) {
                ...
                pw.println("RELAX INFEASIBLE | Lower bound: " + lowerbound
                        + " | Upper bound: " + upperbound + " | Gap: "
                        + ((upperbound - lowerbound) / upperbound) + " | BB Depth: "
                        + depth + " | " + routes.size() + " routes");
                pw.flush();
            ...
            if (branching.lowestValue > upperbound) {
                ...
                System.out.println("CUT | Lower bound: " + lowerbound
                        ...
                        + " routes");
                pw.println("CUT | Lower bound: " + lowerbound
                        + " | Upper bound: " + upperbound + " | Gap: "
                        + ((upperbound - lowerbound) / upperbound) + " | BB Depth: "
                        + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                        + " routes");
                ...
                for (route r : routes) {
                    if (r.getQ() > 1e-6) { // we consider only the routes in the current local solution
                ...
                for (i = 0; i < userParam.nbclients + 2; i++) {
                    for (j = 0; j < userParam.nbclients + 2; j++) {
                        ...
                        if ((coef > 1e-6)
                            ...
                            System.out.println("i= " + i + " ,j= " + j + " = " + coef);
                            pw.println("找到分数值的边 i= " + i + " ,j= " + j + " = " + coef);
                            pw.flush();
                            feasible = false;
                ...
                if (feasible) {
                    if (branching.lowestValue < upperbound) { // new incumbant feasible solution!
                        ...
                        System.out.println("OPT | Lower bound: " + lowerbound
                                ...
                                + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                                + " routes");
                        pw.println("OPT | Lower bound: " + lowerbound
                                + " | Upper bound: " + upperbound + " | Gap: "
                                + ((upperbound - lowerbound) / upperbound) + " | BB Depth: "
                                + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                                + " routes");
                        ...
                        System.out.println("FEAS | Lower bound: " + lowerbound
                                ...
                                + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                                + " routes");
                    pw.println("FEAS | Lower bound: " + lowerbound
                            + " | Upper bound: " + upperbound + " | Gap: "
                            + ((upperbound - lowerbound) / upperbound) + " | BB Depth: "
                            + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                            + " routes");
                    ...
                    System.out.println("INTEG INFEAS | Lower bound: " + lowerbound
                            ...
                            + " routes");
                    System.out.flush();
                    pw.println("INTEG INFEAS | Lower bound: " + lowerbound
                            + " | Upper bound: " + upperbound + " | Gap: "
                            + ((upperbound - lowerbound) / upperbound) + " | BB Depth: "
                            + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                            + " routes");
                    ...
                    boolean ok;
                    ok = BBnode(userParam, nodeRoutes, newnode1, bestRoutes, depth + 1, isOnePass, isMaxcnt,isPrediction);
                    ...
                    ok = BBnode(userParam, nodeRoutes2, newnode2, bestRoutes, depth + 1, isOnePass, isMaxcnt,isPrediction);
                    nodeRoutes2 = null;
